...
function initSortable(id, group) {
	...
	if(typeof window.ogSortable == 'undefined') {
        window.ogSortable = [];
	}
	window.ogSortable[id] = new Sortable(el, {
		group : group,
        onUpdate : function (event) {
			var input = el.getAttribute('data-input');
            input = document.getElementById(input);
            var sort = [];

			var cnt = event.srcElement.children.length;
			for (var i=0; i<cnt; i++) {
                sort.push(event.srcElement.children[i].getAttribute('data-id'));
			}
            input.value = sort.join(',');
        }
	});
}